...
// 编译： gcc maze4.c -o maze -lncurses -lm
// 
// 操作： 
// 方向键 : 移动光标 
// z : 画墙 x : 画路 e : 橡皮 
...
// 颜色定义
enum { 
    C_WALL = 1, C_PATH, C_START, C_END, C_ALGO, C_USER, C_CURSOR, C_BG, C_DEAD, C_AI 
};
        ...
        for (int j = 0; j < size; j++) {
            ...
            if (use_color) {
                if (is_ai) {
                    ...
                    ch = ACS_ULCORNER;
                    is_special = 1;
                } else if (is_start) { color = C_START; ch = 'S'; is_special = 1; }
                else if (is_end) { color = C_END; ch = 'E'; is_special = 1; }
                else if (visited[i][j] == 2) { color = C_DEAD; ch = ACS_BULLET; is_special = 1; }
                else if (maze[i][j] == 1) { color = C_WALL; ch = ACS_CKBOARD; is_special = 1; }
                else { color = C_PATH; ch = ACS_HLINE; }
...
int user_solve_mode() {
    ...
    user_path[1][1] = 1;
    timer_running = 1; start_time = time(NULL);
    draw_maze();
...
void sprinkle_inspiration() {
    ...
    if (density < 0) density = 0; if (density > 100) density = 100;
...
void save_progress() {
    ...
    data.cursor_r = cursor_r; data.cursor_c = cursor_c;
...
int load_progress() {
    ...
    cursor_r = data.cursor_r; cursor_c = data.cursor_c;
    ...
    // 重新调整窗口大小 (因为size可能变了)
    // 这在ncurses中比较麻烦，简单起见我们不重新创建窗口，但绘制时会适配
    // 实际应重新计算布局，这里暂略，假设size不变或略微变化
...
void race_mode_run() {
    ...
    race_mode = 1; race_user_done = 0; race_algo_done = 0; user_solving = 0;
    reset_paths();
    cursor_r = 1; cursor_c = 1; user_path[1][1] = 1;
    Pos stack[MAX_SIZE*MAX_SIZE]; int top = -1;
    stack[++top] = (Pos){1,1}; visited[1][1] = 1;
    ai_r = 1; ai_c = 1; // AI 初始位置

    int algo_winner = 0, user_winner = 0;
    start_time = time(NULL); timer_running = 1;
    ...
    while (1) {
        ...
        if (!user_winner) {
            ...
            switch (ch) {
                case KEY_UP: case 'w': case 'W': nr--; break;
                case KEY_DOWN: case 's': case 'S': nr++; break;
                case KEY_LEFT: case 'a': case 'A': nc--; break;
                case KEY_RIGHT: case 'd': case 'D': nc++; break;
            }
            if (nr>=1&&nr<=size-2&&nc>=1&&nc<=size-2&&maze[nr][nc]==0) {
                cursor_r = nr; cursor_c = nc; user_path[nr][nc] = 1;
...
int main() {
    ...
    printw("Maze Size (3-50): "); refresh();
    int internal; echo(); scanw("%d", &internal); noecho();
    if (internal < 3) internal = 3; if (internal > 50) internal = 50;
    size = internal + 2;
    ...
    init_maze(); reset_paths();
    cursor_r = 1; cursor_c = 1;
    timer_running = 0;

    draw_maze(); show_info("Welcome");
    ...
    while ((ch = wgetch(main_win)) != 'q') {
        if (user_solving) {
            ...
            switch (ch) {
                case KEY_UP: case 'w': case 'W': nr--; moved=1; break;
                case KEY_DOWN: case 's': case 'S': nr++; moved=1; break;
                case KEY_LEFT: case 'a': case 'A': nc--; moved=1; break;
                case KEY_RIGHT: case 'd': case 'D': nc++; moved=1; break;
            }
            if (moved) {
                if (nr>=1&&nr<=size-2&&nc>=1&&nc<=size-2&&maze[nr][nc]==0) {
                    cursor_r = nr; cursor_c = nc; user_path[nr][nc] = 1;
                    if (nr==size-2 && nc==size-2) {
                        ...
                        show_info("SOLVED!"); draw_maze(); napms(2000); continue;
                    }
                }
                draw_maze(); show_info("Solving..."); continue;
            }
        ...
        switch (ch) {
            ...
            case 'z': 
                ...
                maze[cursor_r][cursor_c] = 1; reset_paths(); break;
            case 'x': case 'e': 
                if ((cursor_r==1&&cursor_c==1) || (cursor_r==size-2&&cursor_c==size-2)) break;
                maze[cursor_r][cursor_c] = 0; reset_paths(); break;
            case 's': reset_paths(); draw_maze(); solve_maze_internal(1); break;
            case 'c': show_info(maze_solvable() ? "Solvable" : "No Path"); napms(1500); break;
            case 'u': if (maze_solvable()) user_solve_mode(); else show_info("No Path"); break;
            case 'i': sprinkle_inspiration(); break;
            case 'r': init_maze(); reset_paths(); cursor_r=1; cursor_c=1; timer_running=0; timed_out=0; race_mode=0; show_info("Reset"); napms(1000); break;
            case 'b': save_progress(); napms(1000); break;
            case 'l': if (load_progress()) napms(1000); break;
            case 'a': race_mode_run(); break;
        }
        check_timeout();
        if (timed_out) {
            reset_paths(); solve_maze_internal(1);
            show_info("Timed Out"); timed_out = 0; timer_running = 0; napms(2000);
        }
        draw_maze(); show_info(NULL);
    }
    delwin(main_win); delwin(info_win); endwin(); return 0;
}
