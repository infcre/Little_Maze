...
void race_mode_run() { 
    ...
    wtimeout(main_win, 100); // 恢复游戏循环中的超时设置 
    race_mode = 0; 
    timer_running = 0; 
    set_stage(2); 
} 
...
// 模块化：主游戏循环 
void game_loop() { 
    wtimeout(main_win, 100); // 设置100毫秒超时，解决计时器卡死问题 
    int ch; 
    while ((ch = wgetch(main_win)) != 'q') { 
        if (ch == ERR) ch = 0; // 如果是超时返回的ERR，赋值为0避免误触发
        ...
        switch (ch) { 
            case KEY_UP: 
                if (cursor_r > 1) cursor_r--; 
                break; 
            case KEY_DOWN: 
                if (cursor_r < size-2) cursor_r++; 
                break; 
            case KEY_LEFT: 
                if (cursor_c > 1) cursor_c--; 
                break; 
            case KEY_RIGHT: 
                if (cursor_c < size-2) cursor_c++; 
                break; 
            case 'z': 
            ...
            case 'r': 
                init_maze(); 
                reset_paths(); 
                cursor_r=1; cursor_c=1; timer_running=0; timed_out=0; race_mode=0; 
                show_info("Reset"); 
                set_stage(0); 
                napms(1000); 
                break; 
